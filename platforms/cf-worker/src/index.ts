...
const scheduled = async (event: ScheduledEvent, env: Env, ctx: ExecutionContext) => {
	await (env.SCHEDULER.get(env.SCHEDULER.idFromName(SINGELTON)) as unknown as SchedulerDO).processQueue();
	await (env.SCHEDULER.get(env.SCHEDULER.idFromName(SINGELTON)) as unknown as SchedulerDO).processPendingTransactions();
};
